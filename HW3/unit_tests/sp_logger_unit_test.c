...
//Only Error, Warning and Info messages should be printed in Warning level
...
static bool basicLoggerArgumentTest() {
	...
	ASSERT_TRUE(spLoggerCreate(testFile,SP_LOGGER_INFO_WARNING_ERROR_LEVEL) == SP_LOGGER_SUCCESS);
	ASSERT_TRUE(spLoggerPrintError(0,"sp_logger_unit_test.c",__func__,__LINE__) == SP_LOGGER_INVAlID_ARGUMENT); // null message
	ASSERT_TRUE(spLoggerPrintError("MSGA","sp_logger_unit_test.c",__func__,-10) == SP_LOGGER_INVAlID_ARGUMENT); // negative line number
	ASSERT_TRUE(spLoggerPrintError("MSGB","sp_logger_unit_test.c",0,__LINE__) == SP_LOGGER_INVAlID_ARGUMENT); // null function
	spLoggerDestroy();
	return true;
}
...
static bool basicLoggerDestroyUndefinedTest() {
	...
	ASSERT_TRUE(spLoggerCreate(testFile,SP_LOGGER_INFO_WARNING_ERROR_LEVEL) == SP_LOGGER_SUCCESS);
	spLoggerDestroy();
	ASSERT_TRUE(spLoggerPrintError("MSGC","sp_logger_unit_test.c",__func__,__LINE__) == SP_LOGGER_UNDIFINED); // null logger
	return true;
}
///*
int main() {
